...
// ==================== Write 函数（自动恢复改为调用基类 EnableOutput） ====================
int NoAudioCodec::Write(const int16_t* data, int samples) {
    // 自动恢复：如果输出被禁用，通过基类的 EnableOutput 重新启用
    if (!output_enabled_ && tx_handle_ != nullptr) {
        ESP_LOGI(TAG, "Output disabled, re-enabling via EnableOutput");
        EnableOutput(true);  // 调用基类的 EnableOutput，它会操作硬件并更新标志
    }
