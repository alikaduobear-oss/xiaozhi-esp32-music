...
void Application::CheckNewVersion(Ota &ota)
{
    ...
    while (true)
    {
        ...
        {
            Alert(Lang::Strings::OTA_UPGRADE, Lang::Strings::UPGRADING, "happy", Lang::Sounds::P3_WELCOME);
            ...
            if (!upgrade_success)
            {
                ...
                Alert(Lang::Strings::ERROR, Lang::Strings::UPGRADE_FAILED, "sad", Lang::Sounds::P3_WELCOME);
                vTaskDelay(pdMS_TO_TICKS(3000));
            }
            else
            {
...
{
    // 暂时只播放欢迎音效，数字音效因缺少 0.p3 ~ 9.p3 文件已注释
    Alert(Lang::Strings::ACTIVATION, message.c_str(), "happy", Lang::Sounds::P3_WELCOME);

    /*
    // 数字音效（需要 0.p3 ~ 9.p3 文件）
    struct digit_sound
    {
        char digit;
    ...
    protocol_->OnIncomingJson([this, display](const cJSON *root)
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsString(status) && cJSON_IsString(message) && cJSON_IsString(emotion)) {
                Alert(status->valuestring, message->valuestring, emotion->valuestring, Lang::Sounds::P3_WELCOME);
            } else {
